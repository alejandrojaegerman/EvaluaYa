import { createFileRoute } from "@tanstack/react-router";
import { useSuspenseQuery } from "@tanstack/react-query";

import {
  buildLiveSchemas,
  EN_PATH,
  ES_PATH,
  LIVE_META,
  LiveQuakesPage,
  quakeFeedQuery,
} from "@/components/LiveQuakesPage";
import { absoluteUrl } from "@/lib/site";

export const Route = createFileRoute("/earthquake-in-venezuela-today")({
  loader: ({ context }) => context.queryClient.ensureQueryData(quakeFeedQuery),
  head: ({ loaderData }) => {
    const { title, description } = LIVE_META.en;
    const updatedAt = loaderData?.updatedAt ?? new Date().toISOString();
    return {
      meta: [
        { title },
        { name: "description", content: description },
        { property: "og:title", content: title },
        { property: "og:description", content: description },
        { property: "og:type", content: "website" },
        { property: "og:url", content: absoluteUrl(EN_PATH) },
        { property: "og:locale", content: "en_US" },
        { name: "twitter:card", content: "summary" },
        { name: "twitter:title", content: title },
        { name: "twitter:description", content: description },
      ],
      links: [
        { rel: "canonical", href: absoluteUrl(EN_PATH) },
        { rel: "alternate", hrefLang: "es", href: absoluteUrl(ES_PATH) },
        { rel: "alternate", hrefLang: "en", href: absoluteUrl(EN_PATH) },
        { rel: "alternate", hrefLang: "x-default", href: absoluteUrl(ES_PATH) },
      ],
      scripts: buildLiveSchemas("en", EN_PATH, updatedAt).map((schema) => ({
        type: "application/ld+json",
        children: JSON.stringify(schema),
      })),
    };
  },
  component: EarthquakeTodayPage,
});

function EarthquakeTodayPage() {
  const { data: feed } = useSuspenseQuery(quakeFeedQuery);
  return <LiveQuakesPage feed={feed} />;
}
